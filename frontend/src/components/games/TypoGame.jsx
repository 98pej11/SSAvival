...
export default function TypoGame() {
  ...
  return (
    <Box className='typo' sx={{width:'100%'}}>
      <canvas
        width={1000}
        height={1000}
        ...
        <img className='samsung' src={correctTypos} width='100%' height='100%'/>
      </canvas>
      <Box>
        <Box sx={{...styles,right:'80px', top:'440px', color:'red'}}>{foundSpots.length}</Box>
        <Box sx={{...styles,right:'63px', top:'448px'}}>/</Box>
        <Box sx={{...styles,right:'50px', top:'465px'}}>{typoSpots.length}</Box>
      </Box>
    </Box>
  )
}
