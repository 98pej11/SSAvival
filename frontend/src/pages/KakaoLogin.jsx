...
//   const value = "";
    ...
    const registCheck = async () => {
        // const code = new URL(window.location.href).searchParams.get('code')
        await axios.get
            (`http://localhost:8084/user/kakao/check?email=${email}`) 
            .then((res) => {
    ...
    //     const code = new URL(window.location.href).searchParams.get('code')
    //     await axios.get
    //         (`http://localhost:8084/user/kakao/check?code=${code}`) 
    //         .then((res) => {
    //             console.log("여기axios 잘 넘어왔나 확인");
    //             console.log(res);
    //             console.log(res.data.exist);
    ...
    const regist = async () => {
        ...
        console.log(user);
        await axios
        .post(`http://localhost:8084/user/kakao/regist`, 
        JSON.stringify(user),{
            headers:{
...
//   }, []);
  ...
  return (
    <Pages>
        <div>
            <label>닉네임</label>
            <input type="text" placeholder="닉네임 중복 불가." value={nickname} onChange={handleNicknameChange}></input>
        </div>
        <div>
            <label>지역</label>
            ...
            <button onClick={regist}>제출</button><br/>
